          ...
          {Array.from(new Set(products.map((item) => item.category)))
            ...
            .map((category, index) => (
              <TouchableOpacity
                ...
                <Text
                  className={`${selectedCategory === category ? 'text-white' : 'text-gray-700'} text-[15px]`}>
                  {category}
                </Text>
              </TouchableOpacity>
            ))}
            ...
            {filteredProducts.slice(0, 3).map((item, index) => (
              <TouchableOpacity
                ...
                <View className="flex flex-col justify-center items-center">
                  <Text
                    className={`text-2xl font-Pretendard-Medium ${index === 0 ? 'text-[#AE9717]' : index === 1 ? 'text-[#7A7879]' : 'text-[#AA5F30]'}`}>
                    {index + 1}
                  </Text>
                </View>
                {/* Product Image */}
                ...
                <View className="flex flex-col justify-between w-[70%]">
                  <Text
                    className="text-xl font-pretendard-light"
                    numberOfLines={1}
                    ellipsizeMode="tail">
                    {item.name}
                  </Text>
                  <Text
                    className="text-l font-pretendard-light"
                    numberOfLines={1}
                    ellipsizeMode="tail">
                    {item.description}
                  </Text>
